...
static int16_t meastTemp[MAX_ADC_CHANNEL] = {220,220};
//static uint8_t outSideSensorType = SENSOR_TYPE_3380_10K;


void mod_ntc3380_outsideSensorType(uint8_t type)
{
...
bool measAdcRightCheck(uint16_t measAdcDat)
{
...
const uint16_t B3380_10k_Table[110]=
{    //10k 上拉电阻
...
//const uint16_t B3950_10k_Table[110]=
//{    //10k 上拉电阻
...
const uint16_t B3950_5k_Table[110]=
{
...
int16_t Mod_SearchNTCTable(uint16_t _In_Adc_AvgValue,uint8_t tableType)
{
    ...
    {
        pResistorTable=(const uint16_t*)B3950_5k_Table;
    }
//    else if(SENSOR_TYPE_3950_10K == tableType)
//    {
//        pResistorTable=(const uint16_t*)B3950_10k_Table;
//    }
    else
    {
...
uint8_t  ntcError;
uint8_t mod_ntc3380_error(void)
{
    return ntcError;
}
...
int16_t mod_ntc3380_getMeasTemp(uint8_t sersonType)
{
    int16_t Temperature;
...
{  
    switch(adcStatus)
    {
        ...
        case ADC_STATUS_RUN:
        {
            ...
            if(bsp_ntc_getAdc(&getAdcResult))
            {
                if(measAdcRightCheck(getAdcResult))
                {
                    ntcError &= ~INSIDE_ERROR;
                    meastTemp[INSIDE_SERSON] = Mod_SearchNTCTable(getAdcResult,SENSOR_TYPE_3380_10K);
                }
                else
                {
                    ntcError |= INSIDE_ERROR;
